...
// @ts-ignore
  ...
  try {
    ...
    3. CITACIÓN (OBLIGATORIO): Cita siempre el temario oficial usando el formato de enlace: [MATERIA X.Y](citation://MATERIA/X.Y). Ejemplos: [MAT 4.2](citation://MAT/4.2) para Ecuaciones, [BIO 3.1](citation://BIO/3.1) para Fotosíntesis, [HIS-M 6.1](citation://HIS-M/6.1) para Revolución Mexicana, [GEO 1.1](citation://GEO/1.1) para Espacio Geográfico. IMPORTANTE: Incluye siempre al menos una cita por explicación. NUNCA escribas solo el texto entre corchetes sin el enlace.
    4. DIAGRAMAS (REGLA DE ORO): Para temas complejos, genera diagramas Mermaid usando \`\`\`mermaid\`\`\` con 'flowchart TD' o 'flowchart LR'. NUNCA uses acentos (á,é,í,ó,ú), eñes (ñ), paréntesis (), signos de interrogación ¿?, exclamación ¡!, comas, dos puntos ni símbolos matemáticos dentro de los nodos. Usa SOLO letras A-Z, números y espacios. CRÍTICO MERMAID: Si vas a generar más de un diagrama en la misma respuesta, aplica la misma regla sin excepción a TODOS. Nunca generes un segundo diagrama con acentos, ñ o paréntesis aunque el primero haya salido limpio. Ejemplo: En lugar de "Historia de México (Revolución)", usa "Historia de Mexico Revolucion".
