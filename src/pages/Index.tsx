...
const Index = () => {
  ...
  const [copiadoIndex, setCopiadoIndex] = useState(false);

  const referralCodeIndex = profile?.referral_code ||
    ((profile?.name || '').replace(/\s+/g, '').toUpperCase().slice(0, 4).padEnd(4, 'X') +
     (profile?.id || '').slice(0, 4).toUpperCase());

  const copiarLinkIndex = async () => {
    const link = `https://cyberedumx.com/auth?ref=${referralCodeIndex}`;
    try {
      ...
      {user && (
        <div className="mx-4 mb-4 bg-green-500/10 border border-green-500/30 rounded-2xl p-4">
          ...
          <div className="flex gap-2">
            <input
              value={`https://cyberedumx.com/auth?ref=${referralCodeIndex}`}
              readOnly
              className="flex-1 bg-gray-800/60 text-white rounded-xl px-3 py-2 text-xs border border-green-500/20"
            />
            <button
              onClick={copiarLinkIndex}
              className={`px-4 py-2 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${
                copiadoIndex ? 'bg-green-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
              {copiadoIndex ? '✅ ¡Copiado!' : '📋 Copiar'}
            </button>
          </div>
        </div>
      )}
