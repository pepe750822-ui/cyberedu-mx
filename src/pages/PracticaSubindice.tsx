...
export default function PracticaSubindice() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-24 cyber-grid">
      ...
      {/* WhatsApp button — hidden on mobile during active quiz */}
      <a
        href="https://wa.me/5215523269241"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Únete al grupo de WhatsApp"
        className={`fixed bottom-6 right-4 z-[99] no-underline flex items-center gap-2 group ${quiz.phase !== "idle" ? "hidden md:flex" : "flex"}`}
      >
        <span className="text-[10px] font-black uppercase tracking-wide text-white bg-green-600/90 px-3 py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg">
          ¿Dudas? WhatsApp
        </span>
        <span
          style={{
