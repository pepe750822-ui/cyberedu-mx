...
const PlanEstudioDiario = () => {
    ...
    // Calculation Logic for the Daily Plan
    const dailyPlan = useMemo(() => {
        ...
        // 2. Pick 3 weak areas (lowest progress)
        const weakAreas = areaStats
            .sort((a, b) => a.progress - b.progress)
            .slice(0, 3);

        // 3. Find 3 specific videos (next unseen in those areas)
        const recommendedVideos: { video: Video; area: Area }[] = [];

        weakAreas.forEach(({ area }) => {
            const nextVideo = area.videos.find(v => !isViewed(v.id));
            if (nextVideo) {
                recommendedVideos.push({ video: nextVideo, area });
            }
        });

        // Fallback if some areas are done
        if (recommendedVideos.length < 3) {
            areas.forEach(area => {
                if (recommendedVideos.length >= 3) return;
                const nextVideo = area.videos.find(v => !isViewed(v.id) && !recommendedVideos.some(r => r.video.id === v.id));
                                ...
                                {dailyPlan.map(({ video, area }, index) => (
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Recomendado</span>
                                        </div>
                            ...
                            <div className="mt-10 p-6 bg-primary/5 rounded-3xl border border-primary/10 flex items-start gap-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Hemos seleccionado estas clases porque son las áreas donde tienes mayor oportunidad de crecimiento.
                                        Completar hoy estos 45 minutos te pondrá por delante del 85% de los aspirantes.
                                    </p>
                                </div>
                            </div>
