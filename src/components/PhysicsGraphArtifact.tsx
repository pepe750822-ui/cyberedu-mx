...
const PhysicsGraphArtifact: React.FC<PhysicsGraphArtifactProps> = ({
  initialV = 0,
  initialA = 1,
  initialX = 0,
  ...
  const data = useMemo(() => {
    ...
    for (let t = 0; t <= tLimit; t += 0.5) {
      const x = initialX + v * t + 0.5 * a * Math.pow(t, 2);
      const vel = v + a * t;
      points.push({ t, x: parseFloat(x.toFixed(2)), v: parseFloat(vel.toFixed(2)) });
    }
    return points;
  }, [v, a, tLimit, initialX]);

  // Current position for the visualizer
  const currentPos = useMemo(() => {
    return initialX + v * currentTime + 0.5 * a * Math.pow(currentTime, 2);
  }, [currentTime, v, a, initialX]);
    ...
    <div className="bg-slate-950 rounded-3xl border border-white/10 overflow-hidden shadow-2xl my-4 font-sans">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-white/10 bg-slate-900/50">
        ...
        <div>
          ...
          <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">Gráficas de Movimiento (x vs t / v vs t)</p>
        </div>
      </div>
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
            <label className="flex justify-between text-[10px] font-black text-white/50 uppercase tracking-widest mb-2">
              Velocidad Inicial (v₀): <span className="text-indigo-400">{v} m/s</span>
            </label>
            <input 
          ...
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
            <label className="flex justify-between text-[10px] font-black text-white/50 uppercase tracking-widest mb-2">
              Aceleración (a): <span className="text-emerald-400">{a} m/s²</span>
            </label>
            <input 
          ...
          <div className="relative h-24 bg-slate-900 rounded-2xl border border-white/5 overflow-hidden flex items-end px-4 pb-2">
            ...
            <motion.div 
              ...
              animate={{ x: `${(currentPos % 100)}%` }}
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
            >
              <div className="h-8 w-12 bg-indigo-500 rounded-lg shadow-lg shadow-indigo-500/40 flex items-center justify-center">
                <MoveRight className="h-5 w-5 text-white" />
              </div>
              <div className="text-[8px] font-bold text-white/50 mt-1 uppercase tracking-tighter">Objeto</div>
            </motion.div>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${
                isPlaying ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20'
              }`}
            >
        ...
        <div className="space-y-4">
          <div className="h-48 bg-white/[0.02] rounded-2xl border border-white/5 p-2">
            <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-2 px-2">Posición vs Tiempo (x-t)</p>
            <ResponsiveContainer width="100%" height="90%">
              <LineChart data={data} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ fontSize: '10px', color: '#818cf8', fontWeight: 'bold' }}
                />
          ...
          <div className="h-48 bg-white/[0.02] rounded-2xl border border-white/5 p-2">
            <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-2 px-2">Velocidad vs Tiempo (v-t)</p>
            <ResponsiveContainer width="100%" height="90%">
              <LineChart data={data} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
